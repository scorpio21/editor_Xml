...
<body>
  ...
  <div class="app-header">
    ...
    <div class="lang-select" id="lang-dropdown" data-current="<?= htmlspecialchars(lang()) ?>">
      <button type="button" id="lang-dd-trigger" class="ls-trigger" aria-haspopup="listbox" aria-expanded="false"
        aria-label="<?= htmlspecialchars(t('lang.label')) ?>">
      ...
      <div class="tabs" id="app-tabs">
        <div role="tablist" aria-label="Secciones del editor" aria-orientation="horizontal">
          <button role="tab" id="tab-btn-1" aria-selected="true" aria-controls="tab-panel-1">
            <img class="tab-ico" src="img/ico-home.svg" alt="" aria-hidden="true"><span
              class="tab-text"><?= htmlspecialchars(t('tabs.welcome')) ?></span>
          </button>
          <button role="tab" id="tab-btn-2" aria-selected="false" aria-controls="tab-panel-2">
            <img class="tab-ico" src="img/ico-upload.svg" alt="" aria-hidden="true"><span
              class="tab-text"><?= htmlspecialchars(t('tabs.upload_search')) ?></span>
          </button>
          <?php if (!$isMame): ?>
            <button role="tab" id="tab-btn-3" aria-selected="false" aria-controls="tab-panel-3">
              <img class="tab-ico" src="img/ico-bulk.svg" alt="" aria-hidden="true"><span
                class="tab-text"><?= htmlspecialchars(t('tabs.bulk_delete')) ?></span>
            </button>
          <?php endif; ?>
          <?php if ($isMame): ?>
            <button role="tab" id="tab-btn-4" aria-selected="false" aria-controls="tab-panel-4">
              <img class="tab-ico" src="img/ico-mame.svg" alt="" aria-hidden="true"><span
                class="tab-text"><?= htmlspecialchars(t('tabs.mame_search')) ?></span>
            </button>
          <?php endif; ?>
          <button role="tab" id="tab-btn-5" aria-selected="false" aria-controls="tab-panel-5">
            <img class="tab-ico" src="img/ico-dedupe.svg" alt="" aria-hidden="true"><span
              class="tab-text"><?= htmlspecialchars(t('tabs.dedupe_region')) ?></span>
          </button>
          <button role="tab" id="tab-btn-6" aria-selected="false" aria-controls="tab-panel-6">
            <img class="tab-ico" src="img/ico-home.svg" alt="" aria-hidden="true"><span
              class="tab-text"><?= htmlspecialchars(t('tabs.search_external')) ?></span>
          </button>
          ...
          <button role="tab" id="tab-btn-9" aria-selected="false" aria-controls="tab-panel-9">
            <img class="tab-ico" src="img/ico-dedupe.svg" alt="" aria-hidden="true"><span
              class="tab-text">Duplicados</span>
          </button>
        </div>
        ...
        <section role="tabpanel" id="tab-panel-1" aria-labelledby="tab-btn-1">
          ...
          <p>
            <button type="button" class="secondary"
              onclick="openHelpModal()"><?= htmlspecialchars(t('welcome.help_btn')) ?></button>
          </p>
        </section>
        ...
        <?php if ($isMame): ?>
          <section role="tabpanel" id="tab-panel-4" aria-labelledby="tab-btn-4" hidden>
            ...
            <form method="get" class="search-form">
              ...
              <input id="q_mame" name="q" type="text" value="<?= htmlspecialchars($_GET['q'] ?? '') ?>"
                placeholder="<?= htmlspecialchars(t('mame.input_placeholder')) ?>">
        ...
        <section role="tabpanel" id="tab-panel-9" aria-labelledby="tab-btn-9" hidden>
          <h3>Gestión de Duplicados</h3>
          <?php if ($xml): ?>
            <?php include __DIR__ . '/partials/sections/duplicate-manager.php'; ?>
          <?php else: ?>
            <p class="hint">Primero carga un XML para usar esta herramienta.</p>
          <?php endif; ?>
        </section>
